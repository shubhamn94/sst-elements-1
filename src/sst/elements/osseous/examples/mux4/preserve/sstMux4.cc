...
sstMux4::sstMux4(SST::ComponentId_t id, SST::Params& params) :
   ...
   //set our clock 
   clock_handler = new Clock::Handler<sstMux4>(this, &sstMux4::clockTick); 
   ...
   ArielRtlLink = configureLink("ArielRtllink", new Event::Handler<sstMux4>(this, &sstMux4::handleArielEvent)); 

    // Find all the components loaded into the "memory" slot
    // Make sure all cores have a loaded subcomponent in their slot
    cacheLink = loadUserSubComponent<Interfaces::SimpleMem>("memory", ComponentInfo::SHARE_NONE, timeConverter, new SimpleMem::Handler<sstMux4>(this, &sstMux4::handleMemEvent));
    if(!cacheLink) {
       ...
       cacheLink = loadAnonymousSubComponent<Interfaces::SimpleMem>(interfaceName, "memory", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
               interfaceParams, timeConverter, new SimpleMem::Handler<sstMux4>(this, &sstMux4::handleMemEvent));
...
sstMux4::~sstMux4() {
    output.verbose(CALL_INFO, 1, 0, "sstMux4 destructor called!");
    delete cmodel;
}
...
void sstMux4::init(unsigned int phase) {
...
bool sstMux4::clockTick( SST::Cycle_t currentCycle ) {
...
void sstMux4::handleArielEvent(SST::Event *event) {
    /*
    ...
    */
    ...
    output.verbose(CALL_INFO, 1, 0, "\nsstMux4 RTL Event handle called \n");
...
void sstMux4::handleMemEvent(SimpleMem::Request* event) {
    output.verbose(CALL_INFO, 4, 0, " handling a memory event in Mux4.\n");
    ...
    if(find_entry != pendingTransactions->end()) {
        ...
        output.fatal(CALL_INFO, -4, "Memory event response to sstMux4 was not found in pending list.\n");
        
    delete event;
}

void sstMux4::commitReadEvent(const uint64_t address,
...
void sstMux4::commitWriteEvent(const uint64_t address,
...
void sstMux4::generateReadRequest(RtlReadEvent* rEv) {
...
void sstMux4::generateWriteRequest(RtlWriteEvent* wEv) {
...
uint8_t* sstMux4::getDataAddress() {
    return dataAddress;
}

void sstMux4::setDataAddress(uint8_t* virtAddress){
    dataAddress = virtAddress;
}

uint8_t* sstMux4::getBaseDataAddress(){
    return baseDataAddress;
}

void sstMux4::setBaseDataAddress(uint8_t* virtAddress){
