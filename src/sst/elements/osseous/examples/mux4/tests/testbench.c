...
int main(int argc, char *argv[]) {
  ...
  UInt<1> io_ins_0, io_ins_1, io_ins_2, io_ins_3, io_out;
...
#pragma omp parallel for
  ...
  params->perform_update(false, true, true, true, true, false, false, 10);
  params->storetomem(shmem);
  ctrl_ptr[1] = io_load;
  params->storetomem(shmem);
  ...
  params->perform_update(false, true, true, true, true, true, true, 10000000);
  params->storetomem(shmem);
  ctrl_ptr[1] = io_load;
  params->storetomem(shmem);
  update_RTL_sig(shmem);
