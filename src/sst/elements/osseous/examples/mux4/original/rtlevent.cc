...
void RTLEvent::UpdateRtlSignals(void *update_data, * cmodel, uint64_t& cycles) {
    ...
    if(update_ctrl) {
        UInt<>* rtl_inp_ptr = (UInt<>*)inp_ptr;
        ctrl_ptr = ++rtl_inp_ptr;
        control_sigs(cmodel);
    }
}

void RTLEvent::input_sigs(* cmodel) {

    return;
}

void RTLEvent::control_sigs(* cmodel) {
