...
class Lexer():
    ...
    for file in text:
        if 'O1.h' in file: # soft code later
            break
    file = open(file).read()
    if len(regex.findall('typedef struct',file)) != 1: raise Exception('Cannot determine typedef struct DUT in header.')
    else: 
        p = regex.compile('typedef struct')
        p = p.finditer(file)
        for m in p:
            _p = m.end()
            type = regex.search(r'typedef struct(.*?)\{', file).group(1).strip()
