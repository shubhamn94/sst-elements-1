...
accumulator::accumulator(SST::ComponentId_t id, SST::Params& params) :
	...
	output.init("accumulator-" + getName() + "-> ", 1, 0, SST::Output::STDOUT);
   ...
   //set our clock 
   clock_handler = new Clock::Handler<accumulator>(this, &accumulator::clockTick); 
   ...
   ArielRtlLink = configureLink("ArielRtllink", new Event::Handler<accumulator>(this, &accumulator::handleArielEvent)); 

    // Find all the components loaded into the "memory" slot
    // Make sure all cores have a loaded subcomponent in their slot
    cacheLink = loadUserSubComponent<Interfaces::SimpleMem>("memory", ComponentInfo::SHARE_NONE, timeConverter, new SimpleMem::Handler<accumulator>(this, &accumulator::handleMemEvent));
    if(!cacheLink) {
       ...
       cacheLink = loadAnonymousSubComponent<Interfaces::SimpleMem>(interfaceName, "memory", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
               interfaceParams, timeConverter, new SimpleMem::Handler<accumulator>(this, &accumulator::handleMemEvent));
...
accumulator::~accumulator() {
    output.verbose(CALL_INFO, 1, 0, "accumulator destructor called!");
    delete cmodel;
}
...
void accumulator::init(unsigned int phase) {
...
bool accumulator::clockTick( SST::Cycle_t currentCycle ) {
...
void accumulator::handleArielEvent(SST::Event *event) {
    /*
    ...
    */
    ...
    output.verbose(CALL_INFO, 1, 0, "\naccumulator RTL Event handle called \n");
...
void accumulator::sendArielEvent() {
...
void accumulator::handleMemEvent(SimpleMem::Request* event) {
    ...
    if(find_entry != pendingTransactions->end()) {
        ...
        output.fatal(CALL_INFO, -4, "Memory event response to accumulator was not found in pending list.\n");
        
    delete event;
}

void accumulator::commitReadEvent(const uint64_t address,
...
void accumulator::commitWriteEvent(const uint64_t address,
...
void accumulator::generateReadRequest(RtlReadEvent* rEv) {
...
void accumulator::generateWriteRequest(RtlWriteEvent* wEv) {
...
uint8_t* accumulator::getDataAddress() {
    return dataAddress;
}

void accumulator::setDataAddress(uint8_t* virtAddress){
    dataAddress = virtAddress;
}

uint8_t* accumulator::getBaseDataAddress(){
    return baseDataAddress;
}

void accumulator::setBaseDataAddress(uint8_t* virtAddress){
