...
sstAccumulator::sstAccumulator(SST::ComponentId_t id, SST::Params& params) :
	SST::Component(id)/*, verbosity(static_cast<uint32_t>(out->getVerboseLevel()))*/ {

    bool found;
    RtlAckEv = new ArielComponent::ArielRtlEvent();
    cmodel = new Accumulator;
	output.init("sstAccumulator-" + getName() + "-> ", 1, 0, SST::Output::STDOUT);
   ...
   //set our clock 
   clock_handler = new Clock::Handler<sstAccumulator>(this, &sstAccumulator::clockTick); 
   ...
   ArielRtlLink = configureLink("ArielRtllink", new Event::Handler<sstAccumulator>(this, &sstAccumulator::handleArielEvent)); 

    // Find all the components loaded into the "memory" slot
    // Make sure all cores have a loaded subcomponent in their slot
    cacheLink = loadUserSubComponent<Interfaces::SimpleMem>("memory", ComponentInfo::SHARE_NONE, timeConverter, new SimpleMem::Handler<sstAccumulator>(this, &sstAccumulator::handleMemEvent));
    if(!cacheLink) {
       ...
       cacheLink = loadAnonymousSubComponent<Interfaces::SimpleMem>(interfaceName, "memory", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
               interfaceParams, timeConverter, new SimpleMem::Handler<sstAccumulator>(this, &sstAccumulator::handleMemEvent));
...
sstAccumulator::~sstAccumulator() {
    output.verbose(CALL_INFO, 1, 0, "sstAccumulator destructor called!");
    delete cmodel;
}

void sstAccumulator::setup() {
    cmodel->reset = UInt<1>(1);
	output.verbose(CALL_INFO, 1, 0, "Component is being setup\n");
    
}

void sstAccumulator::init(unsigned int phase) {
...
bool sstAccumulator::clockTick( SST::Cycle_t currentCycle ) {
...
void sstAccumulator::handleArielEvent(SST::Event *event) {
    /*
    ...
    */
    ...
    output.verbose(CALL_INFO, 1, 0, "\nsstAccumulator RTL Event handle called \n");
...
void sstAccumulator::sendArielEvent() {
...
void sstAccumulator::handleMemEvent(SimpleMem::Request* event) {
    output.verbose(CALL_INFO, 4, 0, " handling a memory event in Accumulator.\n");
    ...
    if(find_entry != pendingTransactions->end()) {
        ...
        output.fatal(CALL_INFO, -4, "Memory event response to sstAccumulator was not found in pending list.\n");
        
    delete event;
}

void sstAccumulator::commitReadEvent(const uint64_t address,
...
void sstAccumulator::commitWriteEvent(const uint64_t address,
...
void sstAccumulator::generateReadRequest(RtlReadEvent* rEv) {
...
void sstAccumulator::generateWriteRequest(RtlWriteEvent* wEv) {
...
uint8_t* sstAccumulator::getDataAddress() {
    return dataAddress;
}

void sstAccumulator::setDataAddress(uint8_t* virtAddress){
    dataAddress = virtAddress;
}

uint8_t* sstAccumulator::getBaseDataAddress(){
    return baseDataAddress;
}

void sstAccumulator::setBaseDataAddress(uint8_t* virtAddress){
