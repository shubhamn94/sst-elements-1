...
dut::dut(SST::ComponentId_t id, SST::Params& params) :
   ...
   //set our clock 
   clock_handler = new Clock::Handler<dut>(this, &dut::clockTick); 
   ...
   ArielRtlLink = configureLink("ArielRtllink", new Event::Handler<dut>(this, &dut::handleArielEvent)); 

    // Find all the components loaded into the "memory" slot
    // Make sure all cores have a loaded subcomponent in their slot
    cacheLink = loadUserSubComponent<Interfaces::SimpleMem>("memory", ComponentInfo::SHARE_NONE, timeConverter, new SimpleMem::Handler<dut>(this, &dut::handleMemEvent));
    if(!cacheLink) {
       ...
       cacheLink = loadAnonymousSubComponent<Interfaces::SimpleMem>(interfaceName, "memory", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
               interfaceParams, timeConverter, new SimpleMem::Handler<dut>(this, &dut::handleMemEvent));
...
dut::~dut() {
    output.verbose(CALL_INFO, 1, 0, "dut destructor called!");
    delete cmodel;
}
...
void dut::init(unsigned int phase) {
...
bool dut::clockTick( SST::Cycle_t currentCycle ) {
...
void dut::handleArielEvent(SST::Event *event) {
    /*
...
void dut::handleMemEvent(SimpleMem::Request* event) {
    output.verbose(CALL_INFO, 4, 0, " handling a memory event in TYPE.\n");
    ...
    if(find_entry != pendingTransactions->end()) {
        ...
        output.fatal(CALL_INFO, -4, "Memory event response to dut was not found in pending list.\n");
        
    delete event;
}

void dut::commitReadEvent(const uint64_t address,
...
void dut::commitWriteEvent(const uint64_t address,
...
void dut::generateReadRequest(RtlReadEvent* rEv) {
...
void dut::generateWriteRequest(RtlWriteEvent* wEv) {
...
uint8_t* dut::getDataAddress() {
    return dataAddress;
}

void dut::setDataAddress(uint8_t* virtAddress){
    dataAddress = virtAddress;
}

uint8_t* dut::getBaseDataAddress(){
    return baseDataAddress;
}

void dut::setBaseDataAddress(uint8_t* virtAddress){
